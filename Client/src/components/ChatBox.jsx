...
const ChatBox = () => {
  ...
  useEffect(() => {
    ...
    const fetchSelectedUser = async () => {
      try {
        const response = await axios.get(`https://ownchatapp.onrender.com/api/user/fetchSelectedUser/${selectedUser}`, { withCredentials: true });
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(`https://ownchatapp.onrender.com/api/message/fetchMessages?sender=${userData.userId}&receiver=${selectedUser}`, { withCredentials: true });
        if (response.status === 200) {
